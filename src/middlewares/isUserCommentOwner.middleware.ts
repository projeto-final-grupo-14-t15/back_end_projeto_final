...
const checkCommentOwner = async (req:Request, res:Response, next:NextFunction) :Promise<Response | void> =>{
   ...
   if(Number(comment.author.id) !== Number(res.locals.token.id)){
        throw new AppError(`Comment with id ${commentId} do not belong to you!`, 409);
   }
